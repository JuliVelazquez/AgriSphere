...
@router.get(
    "/api/usuarios/{usuario_id}/asignaciones",
    response_model=AsignacionesResponse
)
async def obtener_asignaciones(
    ...
    if (
        rol_actual == "Usuario"
        and usuario_id != usuario_actual_id
    ):
        raise HTTPException(
            status_code=403,
            detail="No puedes consultar las asignaciones de otro trabajador."
        )
    ...
    resultado_inv = await db.execute(
        select(Invernadero).where(
            Invernadero.nombre.in_(
                access_level
            )
        )
    )
